...
//Two array data, one for left data and other for right side data
var data_lft_top = 0;
var data_rgt_top = 0;
var pre_top = -1;
...
Item.prototype.layoutPosition = function() {
  ...
  if ( layoutOptions.isOriginLeft ) {
	var left = this.position.x + layoutSize.paddingLeft;
	
	//assiging left position on right side div
	if (left > 0) {
		isLeft = false;
	}
    style.left = left + 'px';
  ...
  if ( layoutOptions.isOriginTop ) {
	var _top = this.position.y + layoutSize.paddingTop;
	if (pre_top > _top) {
		pre_top -= 30;
	}
	var _val = _top - pre_top;
	if (pre_top != -1 && _val < 31) {
		var diff = 30 - _val;
		if (isLeft) {
			data_lft_top += diff;
			_top = _top + data_lft_top;
			style.top = _top + 'px';
		} else {
			data_rgt_top += diff;
			_top = _top + data_rgt_top;
			style.top = _top + 'px';
		}
	} else {
		if (isLeft) {
